// Ported from preview/src/fonts.js (never import preview/ — it stays untouched).
// Shared by googleFonts.ts (dynamic Google Fonts loading) and any future
// caller needing the font families a system actually references (e.g. the
// Compare tab's combined multi-column CSS).
const GENERIC_FONT =
 ...
 */
export function fontFamiliesIn(css: string): string[] {
  if (!css) return [];
