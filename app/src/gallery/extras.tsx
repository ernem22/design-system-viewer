...
import { Button, Demo, Icon, usePortalContainer } from "./ui.tsx";
...
function VarVal({ name }: { name: string }) {
  const ref = useRef<HTMLElement | null>(null);
  const [val, setVal] = useState("");
  useEffect(() => {
    try {
      setVal(getComputedStyle(ref.current as HTMLElement).getPropertyValue(name).trim());
    } catch { setVal(""); }
  }, [name]);
...
const Section = ({ id, title, desc, children }: { id: string; title: string; desc: string; children: ReactNode }) => (
...
function InteractiveTableDemo() {
  ...
  const rows = useMemo(() => {
    const r = [...INTERACTIVE_TABLE_BASE];
    r.sort((a, b) => String(a[sort.col]).localeCompare(String(b[sort.col])) * sort.dir);
    return r;
  }, [sort]);
  const toggleSort = (col: number) => setSort((s) => s.col === col ? { col, dir: -s.dir } : { col, dir: 1 });
  const toggleRow = (name: string) => setSelected((s) => s.includes(name) ? s.filter((x) => x !== name) : [...s, name]);
    ...
    <Section id="data-display" title="Data display" desc="Styled with tokens — not in Radix Primitives, built from theme variables.">
      ...
      <Demo title="Multi-select list (selected)">
        <div style={{ minWidth: 220 }}>
          {([["Design tokens", true], ["Component preview", true], ["Compare mode", false], ["Dark variant", false]] as Array<[string, boolean]>).map(([label, init]) => (
    ...
    <Section id="patterns" title="Product patterns" desc="Non-Radix patterns every product needs — hand-built with tokens.">
      ...
      <Demo title="Keyboard shortcuts">
        <table className="dsv-kbd-table" style={{ minWidth: 260 }}>
          <tbody>
            {([["Save", ["⌘", "S"]], ["Search commands", ["⌘", "K"]], ["Undo", ["⌘", "Z"]], ["New project", ["⌘", "N"]], ["Delete", ["⌫"]], ["Toggle sidebar", ["⌘", "B"]]] as Array<[string, string[]]>).map(([label, keys]) => (
    ...
    <Section id="foundation" title="Foundation tokens" desc="Gradient, glow, motion, rhythm, media and layout scales — driven by the expanded token schema.">
      ...
      <Demo title="Media">
        <div className="dsv-media-row">
          {([["wide", "--media-aspect-wide", false], ["standard", "--media-aspect-standard", "subtle"], ["square", "--media-aspect-square", false], ["portrait", "--media-aspect-portrait", "strong"], ["video", "--media-aspect-video", false]] as Array<[string, string, string | boolean]>).map(([label, token, shade]) => (
            <span key={label} className="dsv-stack" style={{ gap: "var(--space-1)", alignItems: "center" }}>
      ...
      <Demo title="Control + icon sizes">
        <div className="dsv-stack" style={{ gap: "var(--space-4)", width: "100%" }}>
          ...
          <div className="dsv-inline" style={{ gap: "var(--space-3)", alignItems: "flex-end", flexWrap: "wrap" }}>
            {([["xs", "var(--size-icon-xs)", 12], ["sm", "var(--size-icon-sm)", 16], ["md", "var(--size-icon-md)", 20], ["lg", "var(--size-icon-lg)", 24], ["xl", "var(--size-icon-xl)", 32]] as Array<[string, string, number]>).map(([s, token, px]) => (
              <span key={s} className="dsv-stack" style={{ gap: "var(--space-1)", alignItems: "center" }}>
      ...
      <Demo title="Composition — widths, offsets, aspects">
        <div className="dsv-stack" style={{ gap: "var(--space-2)", width: "100%" }}>
          ...
          <div className="dsv-inline" style={{ gap: "var(--space-3)", flexWrap: "wrap", alignItems: "flex-end" }}>
            {([[120, "var(--composition-aspect-wide)", "wide"], [96, "var(--composition-aspect-standard)", "standard"], [72, "var(--composition-aspect-square)", "square"], [56, "var(--composition-aspect-portrait)", "portrait"]] as Array<[number, string, string]>).map(([w, ar, label]) => (
              <span key={label} className="dsv-stack" style={{ gap: "var(--space-1)", alignItems: "center" }}>
