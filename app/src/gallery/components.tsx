...
import { Button, Field, Demo, Icon, usePortalContainer } from "./ui.tsx";
...
export function FormsSection() {
  ...
  const [ind, setInd] = useState<boolean | "indeterminate">("indeterminate");
          ...
          <Popover.Portal container={portalContainer}>
            <Popover.Content className="dsv-pop dsv-pop--lg" sideOffset={6}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "var(--space-4)" }}>
                {([["Product", ["Analytics", "Dashboard", "Automation"]], ["Company", ["About", "Careers", "Press"]], ["Resources", ["Docs", "API", "Status"]]] as [string, string[]][]).map(([h, items]) => (
                  <div key={h}>
    ...
    <Section id="navigation" title="Navigation" desc="Menubar, navigation menu, tabs, toolbar.">
      <Demo title="Menubar">
        <Menubar.Root className="dsv-nav">
          {([["File", ["New", "Open…", "Save"]], ["Edit", ["Cut", "Copy", "Paste"]], ["View", ["Zoom in", "Zoom out", "Fullscreen"]]] as [string, string[]][]).map(([m, items]) => (
            <Menubar.Menu key={m}>
        ...
        <div style={{ width: "100%", maxWidth: "var(--space-96)" }}>
          ...
          <div className="dsv-inline" style={{ alignItems: "stretch", gap: "var(--space-2)", flexWrap: "wrap" }}>
            <div className="dsv-shell-side" style={{ background: "var(--color-surface-sunken)", borderRadius: "var(--radius-md)", padding: "var(--space-2)", fontSize: "var(--font-size-xs)", display: "flex", flexDirection: "column", gap: 2 }}>
              {([["Overview", true], ["Projects", false], ["Team", false], ["Settings", false]] as [string, boolean][]).map(([label, on]) => (
...
export const COMPONENT_SECTIONS: Section[] = [
