...
import { Button, Field, Icon, Screen, usePortalContainer } from "./ui.tsx";
import type { ComponentType, ReactNode } from "react";
import type { Section } from "./section.ts";

// size: token scale name (xs/sm/md/lg/xl) — resolves to --size-avatar-*
const Avat = ({ n, size = "md" }: { n: number; size?: "xs" | "sm" | "md" | "lg" | "xl" }) => (
...
const Sep = ({ children }: { children: ReactNode }) => (
...
const Link = ({ children, to = "#screen-login" }: { children: ReactNode; to?: string }) => <a href={to} className="dsv-link" style={{ display: "inline", padding: 0 }}>{children}</a>;
...
function PricingScreen() {
  ...
  const plans: [string, number, string[]][] = [
...
// ─────────────────────────────────────────── Settings
const SettingRow = ({ title, desc, children }: { title: string; desc: string; children: ReactNode }) => (
...
function UploadScreen() {
  const files: [string, string, number][] = [["report.pdf", "2.4 MB", 100], ["data.csv", "812 KB", 100], ["presentation.key", "18 MB", 46]];
  return (
    <div style={{ maxWidth: 520, margin: "0 auto" }}>
      <div className="dsv-card">
...
function ScheduleScreen() {
  ...
  const events: Record<string, [string, string, string][]> = { Tue: [["9:30", "Standup", "success"], ["14:00", "Design review", "info"]], Wed: [["11:00", "1:1 Ada", "warning"]], Thu: [["10:00", "Sprint planning", "info"], ["16:00", "Demo", "success"]] };
  return (
    <div>
...
type ScreenRow = [id: string, label: string, Body: ComponentType, desc: string];

export const SCREEN_SECTIONS: Section[] = (
  [
  ...
  label,
  Comp: () => (
    <Screen id={id} title={label} desc={desc}>
      <Body />
    </Screen>
  ),
}));
