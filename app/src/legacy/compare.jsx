...
export default function Compare({ systems: propSystems }) {
  const [systems, setSystems] = useState(propSystems ?? null);
  ...
  useEffect(() => {
    if (propSystems) {
      setSystems(propSystems);
      setPicked((cur) => {
        const valid = cur.filter((slug) => propSystems.some((s) => s.slug === slug));
        return valid.length ? valid : propSystems.slice(0, 2).map((s) => s.slug);
      });
      return;
    }
    fetchSystems()
