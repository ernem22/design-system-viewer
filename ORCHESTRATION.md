...
| Three Testers dispatched at once all froze mid-turn — identical token counters over minutes while their spinners animated — and the same specs run one at a time completed normally (counter climbing 42K → 72K in 70 seconds) | Each Tester holds a browser session, a served app and a provider stream; serialise them. The frozen counter is the only signal: the spinner is decoration, the counter is the evidence |
| A Tester that had finished its measurement never sent its verdict — it was stuck passing a multi-line `--body` through PowerShell (backtick-n and here-string attempts) | A worker can wedge on reporting, not on the work, and it looks exactly like a hung tool call. Specs carry the boring path: write the body to a file and pass `--body "$(cat <file>)"`, or send a ONE-LINE verdict and put the detail in a `gh pr comment --body-file`. A one-line verdict that lands beats a perfect multi-line one that never does |
| After a rebase, specs still naming the old head produced STEP 0 mismatches twice in one night (tester-92d, tester-93f) — both workers asked or reported `blocked` instead of proceeding | That is the behaviour the rule exists for. Update the anchors in the same pass as the rebase, or write the spec as "the head is whatever `gh pr view <n> --json headRefOid` returns" so it cannot go stale |
| PRs #103, #92 and #93 were opened with base `main` — the repo's default branch — instead of the pipeline base `refactor/full-react-migration`, so the shell refactor merged into a branch the app does not live on, and the two open PRs showed diffs of 245 files / 99 commits (the whole migration) where their real change was three files | The coder template said "a PR against `refactor/full-react-migration`" but gave no command, so `gh pr create` took the default base. Name the base in the command (`gh pr create --base "$BASE"`), and have the closer compare a PR against **its own** `baseRefName` — a wrong-base PR looks merely "1 commit behind" against the pipeline base while its real diff is the whole migration, so the wrong comparison hides the fault instead of reporting it |
